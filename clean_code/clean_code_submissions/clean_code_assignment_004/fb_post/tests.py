...
def test_get_posts_with_more_positive_reactions(post_reaction):
    ...
    # Assert
    assert set(expected_output) == set(post_ids_list)

def test_get_posts_with_no_positive_reactions(post):

    # Arrange
    Reaction.objects.bulk_create([
    Reaction(post_id = 1,reaction = 'SAD',reacted_by_id = 1),
    Reaction(post_id = 1,reaction = 'THUMBS-DOWN',reacted_by_id = 2),
    Reaction(post_id = 1,reaction = 'ANGRY',reacted_by_id = 3),
    ])
    expected_output = []
    ...
    # Assert
    assert expected_output == post_ids_list


def test_get_posts_with_equal_positive_reactions(post):

    # Arrange
    Reaction.objects.bulk_create([
    Reaction(post_id = 1,reaction = 'SAD',reacted_by_id = 1),
    Reaction(post_id = 1,reaction = 'LOVE',reacted_by_id = 2),
    Reaction(post_id = 2,reaction = 'ANGRY',reacted_by_id = 3),
    Reaction(post_id = 2,reaction = 'LIT',reacted_by_id = 1),
    ])
    expected_output = []
...
# def test_get_post_with_valid_details_returns_post_details(post_reaction,
#                                                           comment_reaction,
#                                                           reply):
...
#     for_check_comment_details(expected_output, output,0)
#     for_check_comment_details(expected_output, output,1)
