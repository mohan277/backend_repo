...
class Store:
    ...
    def filter(self, query):
        ...
        def in_operation(self, query):
            for item in self.items_in_store:
                ...
                if query.operation == 'IN':
                    if get_attr in query.value:
                        filtered_items.append(item)

        def eq_operation(self, query):
            for item in self.items_in_store:
                get_attr = getattr(item, query.field)
                if query.operation == 'EQ':
                    if get_attr == query.value:
                        filtered_items.append(item)

        def gt_operation(self, query):
            for item in self.items_in_store:
                get_attr = getattr(item, query.field)
        ...
        def startswith_operation(self, query):
            for item in self.items_in_store:
                get_attr = getattr(item, query.field)
                if query.operation == 'STARTS_WITH':
                    if get_attr.startswith(query.value):
                        filtered_items.append(item)

        def endswith_operation(self, query):
            for item in self.items_in_store:
                get_attr = getattr(item, query.field)
                if query.operation == 'ENDS_WITH':
                    if get_attr.endswith(query.value):
                        filtered_items.append(item)
