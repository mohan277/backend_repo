...
@pytest.fixture
def race_car():
    race_car = RaceCar(color="Red", max_speed=50, acceleration=10,
                       tyre_friction=3)
    return race_car
...
def test_truck_object_creation_when_parameters_passed_returns_truck_object():
    ...
    # Act
    race_car = RaceCar(color="Red", max_speed=250, acceleration=10,
                       tyre_friction=3)
...
def test_accelerate_when_nitro_is_available_returns_extra_speed(race_car):
...
def test_encapsulation_of_protected_attribute_nitro_returns_error(race_car):
    ...
    with pytest.raises(AttributeError) as error_message:
        ...
        # Assert
        assert str(error_message.value) == expected_output
